...
make_ref_similarity_names_for_groups <- function(de_table.ref.marked, the_test_dataset, 
   ...
   # For each group report top pval, give it a name/no similarity and all stepped pvals.
   make_group_name <- function(pvals_to_next, test_group, group_list) {
      if (any(pvals_to_next[1:(length(pvals_to_next)-1)] < pval)) { #lsast is NA
         first_sig_pval <- which(pvals_to_next < pval)[1]
         return(paste0(test_group, ":", paste0(group_list[1:first_sig_pval], collapse = "|"))) 
      }
      return(paste0(test_group, ":no_similarity"))
   }
   
   # First significant pvalue, or just the first if none are sig.
   get_pval <- function(pvals_to_next) {
      if (any(pvals_to_next[1:(length(pvals_to_next)-1)] < pval)) { #lsast is NA
         first_sig_pval <- which(pvals_to_next < pval)[1]
         return(pvals_to_next[first_sig_pval])
      }
      return(pvals_to_next[1]) # if all non-sig, just the first.
   }
   
   
   labels_by_similarity_table <- 
      mwtest_res_table %>% 
      dplyr::arrange(.data$test_group, .data$grouprank ) %>%
      dplyr::group_by(.data$test_group) %>%
      dplyr::mutate(shortlab             = make_group_name( .data$pval_to_next, .data$test_group, .data$group), 
                    pval                 = get_pval(.data$pval_to_next),
                    stepped_pvals        = paste0(.data$group, ":", base::signif(.data$pval_to_next, 3), collapse = ",")) %>%
      dplyr::select(.data$test_group, 
                    .data$shortlab, 
                    .data$pval, 
                    .data$stepped_pvals) %>% base::unique()
