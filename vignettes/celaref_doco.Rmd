...
kable(make_ref_similarity_names_for_groups(de_table.marked.query_vs_ref), digits=50)
```
...
```{r eval=TRUE, cache=FALSE} 
# And get group labels
group_names_table <- make_ref_similarity_names_for_groups(de_table.marked.query_vs_ref)
```
```{r eval=TRUE, echo=FALSE} 
...
kable(make_ref_similarity_names_for_groups( de_table.marked.cortex_vs_hippo) %>% arrange(test_group), 
