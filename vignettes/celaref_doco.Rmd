...
process using a basic dense matrix. 

Sadly celaref does not yet support ths use of hdf5-backed delayedArray counts 
(via the save/loadHDF5SummarizedExperiment functions from HDF5Array package).
Attempting to use these will yeild an error.

So options to process large datasets:

1. Store the counts in a sparse Matrix (via capital M Matrix package)


```{r eval=FALSE}
library(Matrix)
# e.g. given a (hdf5-backed) summarisedExperiment object someSE:
sparse_counts_matrix <- Matrix(counts(someSE), nrow=nrow(someSE),  sparse=TRUE)
```
...
3. For very large datasets, it might be better to shrink the dataset upfront
   (loosing the proportionality of cell types).
