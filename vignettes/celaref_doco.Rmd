...
Query Group | Short Label                        | pval    | stepped_pvals 
------------|------------------------------------|---------|---------------------
...
cluster_3   |cluster_3:no_similarity             |4.10e- 1 | astrocytes_ependymal:0.41,microglia:0.634,oligodendrocytes:0.305,endothelial-mural:0.512,interneurons:0.204,pyramidal SS:NA
...
4. Calls are made on which reference groups to include in the group label.
     ...
     - (b) If there are no significant differences between any steps, report a lack of similarity.
      The pval in this case will be again be the first stepped pvalue - but it isn't significant. 
...
kable(make_ref_similarity_names_for_groups(de_table.marked.query_vs_ref, the_test_dataset='query', the_ref_dataset = "ref"))
```
...
the inner log2FC 95% confidence interval is implemented. Use of 'topconfects' 
is planned [@topconfect], as this is expected to be a better ranking metric.
...
```{r eval=TRUE, echo=FALSE}
kable(label_table.cortex_vs_lacrimal %>% arrange(test_group) ) 
```
