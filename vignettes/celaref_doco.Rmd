...
someone has already determined the cell types (Which they helpfully named 'dunno', 
...
But what cell-types are in each cluster?

We will use a reference of PBMC cells to assign some biological cell types to these clusters.
...
The cell-ranger pipeline produced several different clustering run. 
Going to use the kmeans k=7 set for this example, but comparing different 
cluster-sets to a reference like this might be one way to assess which is most appropriate.

For reference, here are the groups colour-coded on a t-SNE plot in the cell-loupe viewer. 
The number in brackets after the cell group label is the number of cells in the group.

![](images/pbmc4k_cloupe_kmeans7.png) 

First, load the dataset into a SummarisedExperiment object and filter out genes 
...
Setting the num-cores to the 7 - one per group run in parallel. If the computer 
can't support that many simultaneous processes, fewer is fine.
...
only one extra factor is supported.
...
multiple similarities will be reported (in decending order of median rank)
unless a clear (significantly differnt) frontrunner can be flagged.
...
# Longlab is less interesting and may change.
...
On the other hand monocytes and B lympocytes were uniquely
...
Simiarlly, @Farmer2017 have 
recently published a survey of cell types in the mouse lacrimal gland at two 
...
complicated. There was an effience MatrixMarket format for the counts matrix, 
and cell assignment and cluster information are in separate files. So this data 
needs to be convered into the form that *load_se_from_tables* expects.
...
```{r eval=TRUE, echo=FALSE}
kable(head(rowData(dataset_se.P4)))
```

Note that 'ID' is the ensembl gene id. Need to switch this to the gene symbol 
so it matches the Zeisel data. Could equally well use ensembl ids for both.

Gene symbol to ID is *almost* a one to one mapping, so a few genes are lost in 
this step. Calculating the total readcount foreach gene is a simple way of 
...
Now compare the mouse cortex samples to the lacrimal gland. Wouldn't expect 
many cell types in common.
...
The cross-tissue comparison looks very different - as expected, most clusters 
have 'no similarity'.

A few do though - the 'endothelial-mural' cluster from the mouse cortex is most 
similar to the 'Endothelial' and 'Mes 1'(Mesenchymal) groups (just) in the 
lacrimal gland sample. The cortex 'microglia' have their similarity with 
