...
[@Kiselev2018] in that it operates at the cluster-level.
...
And with a low number of 'top' genes.
...
The first 4 columns of output (below) are the most interesting, the last 3 are described at 
...
4. Calls are made on which reference groups to include in the group label.
     - (a) If there is a significant difference (p < 0.01)  between the first and second most similar groups - report on the top ranked reference group. The pval in this case
     will be the first stepped pvalue (which is significant).
     - (b) If there are no significant differences between any steps, report a lack of similarity.
     No p-value is reported - because no call could be made on what group(s) are more similar.
     ...
     Here the reported pvalue describes the 'jump' between the bottom ranked
...
The last 3 columns of the output are usually empty (as they were in this example). 
...
```{r toy_example, message=FALSE, eval=TRUE, cache=FALSE}
...
make_ref_similarity_names_for_groups(de_table.marked.query_vs_ref, the_test_dataset='query', the_ref_dataset = "ref")
```
```{r toy_example3, message=FALSE, warnings=FALSE,  eval=TRUE, echo=FALSE}
kable(make_ref_similarity_names_for_groups(de_table.marked.query_vs_ref, the_test_dataset='query', the_ref_dataset = "ref"), digits=50)
```
...
is planned [@Harrison2018], as this is expected to be a better ranking metric.
...
```{r eval=TRUE, cache=FALSE} 
# And get group labels
group_names_table <- make_ref_similarity_names_for_groups(de_table.marked.query_vs_ref, the_test_dataset='query', the_ref_dataset = "a_demo_reference")
```
```{r eval=TRUE, echo=FALSE} 
...
NB: Its unusual to have anything in the 'similar_non_match' column - it happens 
here due to small size of this toy dataset. Explained in section 
[Assigning labels to clusters](#assigning-labels-to-clusters). 
...
Function *make_ref_similarity_names_for_groups* accepts an optional parameter 'num_steps'.
...
label_table.pbmc4k_k7_vs_Watkins2009PBMCs <- make_ref_similarity_names_for_groups(de_table.pbmc4k_k7_vs_Watkins2009PBMCs, 
...
Monocytes and B lymphocytes were uniquely labelled. 
Clusters 4 and 5 are quite distinct in the t-SNE but both are labelled 
...
Tc and Th lymphocytes can be similar, 
and group 3 reports similarity to both for consideration. Group 1 is large, and
while adjacent to group 3 in the tSNE plot, does not have a clear 'front runner' 
for similarity. Looking at smaller clusters within this group could be justified.
...
kable(make_ref_similarity_names_for_groups( de_table.marked.cortex_vs_hippo , 
                the_test_dataset = "zeisel_sscortex", 
               the_ref_dataset  = "zeisel_ca1hippocampus") %>% arrange(test_group), 
...
label_table.cortex_vs_lacrimal <- 
   make_ref_similarity_names_for_groups( de_table.marked.cortex_vs_lacrimal, 
                                            the_test_dataset = "zeisel_sscortex", 
